document.addEventListener('DOMContentLoaded', () => {
    // --- Elements ---
    const heightUnitRadios = document.querySelectorAll('input[name="heightUnit"]') as NodeListOf<HTMLInputElement>;
    const cmGroup = document.getElementById('cmGroup');
    const ftGroup = document.getElementById('ftGroup');
    const heightCm = document.getElementById('heightCm') as HTMLInputElement;
    ...
    function calculateBMI() {
        const hUnit = (document.querySelector('input[name="heightUnit"]:checked') as HTMLInputElement).value;
        let finalHeightCm = 0;
    ...
    function displayResult(bmi: number, hCm: number, wKg: number) {
        resultCard?.classList.remove('empty');
        resultPlaceholder?.classList.add('hidden');
        resultContent?.classList.remove('hidden');

        // Animate counter
        if (bmiScore) animateCounter(bmiScore, bmi);
        ...
        if (gaugeProgress) {
            (gaugeProgress as any).style.strokeDashoffset = offset;
            (gaugeProgress as any).style.stroke = `var(--${status.id})`;
        }
        ...
        document.querySelectorAll('.indicator').forEach(i => {
            ...
            if (el.dataset.cat === status.label) el.classList.add('active', status.id);
        });

        // Advanced metrics
        const idealMin = (18.5 * ((hCm / 100) ** 2)).toFixed(1);
        const idealMax = (24.9 * ((hCm / 100) ** 2)).toFixed(1);
        if (idealWeight) idealWeight.textContent = `${idealMin} - ${idealMax} kg`;

        const bmr = (10 * wKg) + (6.25 * hCm) - (5 * 25) + 5; // Basal estimate
        if (dailyCal) dailyCal.textContent = `${Math.round(bmr * 1.3)} kcal`;

        if (healthAdvice) healthAdvice.textContent = status.advice;
    }
    ...
    function animateCounter(el: HTMLElement, val: number) {
        let current = 0;
        const target = val;
    ...
    function saveToHistory(bmi: number) {
        const item = {
            bmi: bmi.toFixed(1),
